...
The repository-grounded `M10-interface-audit.md` found that the CLI has the broadest complete workflow coverage, the TUI is operationally narrow, and the Tauri bridge exposes more canonical capability than the Vue adapter currently wires. Existing application/read-model APIs were sufficient for the first interface slice; general post-creation task metadata editing remains the only clear shared application API gap identified by the audit.

M10-001 completed at `8106d75695da31dc5212b236eeeca60040e168b8`. The TUI now supports the normal task lifecycle vertical slice through existing canonical APIs: task creation, queue/detail inspection, non-blocking dispatch/review/revision/accept actions, manual execution refresh, validation/review evidence, previous-implementation-agent revision continuity, explicit acceptance, and application/illegal-action error handling. Independent review passed after resolving the initial event-loop blocking, integration-coverage, and clippy findings. No shared API addition, duplicate lifecycle logic, Controller inference, automatic experience harvesting, or M09 change was introduced.

M10-002 is the current planned slice: add TUI run history/detail and validated worktree/diff inspection using existing canonical run/read and Git/review boundaries. It is intentionally read-oriented and does not bundle reject/cancel/requeue, recovery execution, dependency mutation, or explicit agent selection.

Follow-on M10 work should remain independently reviewable and proceed from actual operator gaps: TUI recovery/lifecycle controls; typed Controller authorization adapter exposure; supervised M08 curation surfaces; one shared general task-metadata update seam; and remaining desktop governance/economy/Plan/Lead parity gaps. Interface work must continue to preserve canonical Rust ownership of facts, legality, authorization, persistence, validation, and workflow transitions.
