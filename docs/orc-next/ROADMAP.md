...
Repository inspection after M07-004 confirms that repeated bounded continuation does not need another engine. Existing `continue_run_inner()` already owns the finite repeated transition loop and `WorkflowPolicy.max_transitions`; M07-001 grants already own finite shared routine-action budget. The next seam is therefore to compose these two existing bounds.

M07-005 is the next task: add one opt-in grant-aware multi-edge Controller workflow API that reuses `continue_run_inner()`. Deterministic/routing edges consume no continuation budget; each persisted Dispatch/Review/Revision edge reuses M07-004 and may consume exactly one grant unit; exhaustion/revocation/invalidity stops before another routine action; existing transition and revision limits remain independent. Acceptance, explicit user gates, and external-wait boundaries stop this supervised continuation path rather than being silently crossed. Existing non-grant workflow APIs retain their current behavior.

M07-005 still does not persist grants across restart, automatically create/replenish grants, redesign recovery, or automate memory capture/maintenance. Those remain separate future decisions after bounded routine continuation is proven.
