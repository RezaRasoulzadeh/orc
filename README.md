...
Agents can be automated or manual. Automated workers execute through supported backends in isolated worktrees; manual agents receive a task packet and task-worktree path, then wait for structured output or a patch. Dispatch, Revise, manual run submission, and manual patch submission all cross the same Orc-owned deterministic validation boundary before becoming Review-ready. A registered agent may have a provider profile directory, model, reasoning effort (`none`, `low`, `medium`, or `high`), capabilities, availability, and quota settings.
