...
Register an agent with `--mode manual`, then dispatch it. Orc creates a waiting external run and prints a task packet. Give that packet to the human or external provider. Complete it with `orc run submit RUN_ID --file OUTPUT`, or submit a Git patch with `orc run submit-patch RUN_ID PATCH`. These manual submission commands currently record the submitted work and move the task to review; they do not yet run Orc-owned validation. Task 9 will establish validation before Review. Use `-` for stdin. Record an unsuccessful attempt with `orc run fail RUN_ID "reason"`.
