...
pub fn format_revision_contract(contract: &RevisionContract) -> String {
    ...
    out.push_str("\n### Required handoff\nReturn JSON {\"completion\":{\"step_results\":[{\"step_id\":\"...\",\"operations_performed\":[\"modify\"],\"affected_files\":[],\"observed\":[],\"verification_passed\":[]}],\"summary\":\"...\"},\"claims\":[{\"blocker_id\":\"...\",\"status\":\"addressed|unresolved\",\"implementation_summary\":\"...\",\"changed_files\":[],\"unresolved_risk\":null}]} with exactly one claim for every active blocker ID. For each completion step, `operations_performed` must echo exactly the persisted step `operations` list in the same order; do not add incidental inspection, command, or verification activity. Record those details only in `observed` or `verification_passed`. Resolved constraints require no claim. `completion` is the same checkpoint evidence contract used by initial implementation; claims add only revision blocker disposition. Do not include validation evidence in a claim \u{2014} automated review owns validation. Keep changes focused.");
    out
}
