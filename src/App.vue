...
const taskActionModal = ref<{ action: string; reason: string; agentId: string } | null>(null)
...
async function taskAction(action: string, suppliedReason?: string) {
  ...
  if (['reject', 'cancel', 'revise', 'add_dependency'].includes(action) && !suppliedReason) { taskActionModal.value = { action, reason: '', agentId: '' }; return }
  if (['dispatch', 'accept', 'requeue'].includes(action) || action === 'remove_dependency') { confirmationModal.value = { title: `${humanStatus(action)} task`, message: `Are you sure you want to ${action.replace('_', ' ')} this task?`, action: () => submitTaskAction(action, suppliedReason, undefined, true, true) }; return }
  await submitTaskAction(action, suppliedReason, undefined)
}
function continueTaskAction() {
  if (!taskActionModal.value) return
  const { action, reason, agentId } = taskActionModal.value
  if (!reason.trim() || (action === 'revise' && !agentId)) return
  confirmationModal.value = { title: `${humanStatus(action)} task`, message: `Are you sure you want to ${action.replace('_', ' ')} this task?`, action: () => submitTaskAction(action, reason, agentId, true, true) }
  taskActionModal.value = null
}
async function submitTaskAction(action: string, suppliedReason?: string, suppliedAgent?: string, confirmed = false, propagateError = false) {
  if (!selectedTask.value || (mutationLoading.value && !confirmed)) return
  const reason = suppliedReason?.trim() || undefined
  const agentId = suppliedAgent?.trim() || undefined
  if (['reject', 'cancel', 'revise', 'add_dependency'].includes(action) && !reason) return
  if (action === 'revise' && !agentId) return
  ...
  try {
    ...
    taskError.value = friendlyError(err)
...
async function toggleAgentAction(action: 'code' | 'review' | 'plan' | 'lead') { if (!selectedAgentDefinition.value) return; try { await api.configureAgentAction(selectedAgentDefinition.value.id, action, !selectedAgentDefinition.value.actions.includes(action)); await refreshAgents() } catch (err: unknown) { agentError.value = friendlyError(err) } }
